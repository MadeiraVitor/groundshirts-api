  ...
  try {
    ...
    const user1 = await prisma.user.create({
      data: {
        ...
        password:
          "$2b$10$K7L1OJ45/4Y2nIvhRVpCe.FPM0xLqc8y8eMONLr9xOhQ1lUPXIEki", // senha: password123
        role: "USER",
      },
    });
    ...
    console.log(
      `✅ 3 usuários criados (${user1.email}, ${user2.email}, ${admin.email})`,
    );
    ...
    const classicTee = await prisma.product.findUnique({
      where: { slug: "classic-tee" },
    });
    const vintageHoodie = await prisma.product.findUnique({
      where: { slug: "vintage-hoodie" },
    });
    const slimJeans = await prisma.product.findUnique({
      where: { slug: "slim-jeans" },
    });
    const sportShorts = await prisma.product.findUnique({
      where: { slug: "sport-shorts" },
    });
    const runningShoes = await prisma.product.findUnique({
      where: { slug: "running-shoes" },
    });
    const summerDress = await prisma.product.findUnique({
    ...
    console.log(
      `✅ 4 pedidos criados (Order IDs: ${order1.id}, ${order2.id}, ${order3.id}, ${order4.id})`,
    );
