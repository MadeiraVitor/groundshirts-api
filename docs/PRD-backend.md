...
Construir uma API REST em Node.js + Fastify + TypeScript que suporte o front-end existente do projeto `groundshirts-app`. A API deverá gerenciar catálogo (produtos e categorias), autenticação, pedidos (checkout), cálculo de frete por CEP, gerenciamento de assinaturas/newsletter e armazenamento de imagens em Supabase Storage. Persistência via Supabase Postgres acessada por Prisma. Autenticação via JWT.
